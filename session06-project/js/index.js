...
let number = document.getElementsByClassName("cart-number");
...
condition.addEventListener("click", function(){
    if(userLogin){
        ...
        Swal.fire({
            icon: 'error',
            title: 'Đăng nhập để kiểm tra giỏ hàng',
...
function addToCart(productId) {
    ...
    if (!userLogin) {
        Swal.fire({
            icon: 'error',
            title: 'Hãy đăng nhập để mua hàng',
            showConfirmButton: false,
            timer: 1500 
        })
        return;
    ...
    for(let l = 0;l < users.length;l++){
        if(users[l].id = userLogin.id){
            if(users[l].lock === false){
                Swal.fire({
                    icon: 'error',
                    title: 'Tài khỏa của bạn đã bị khóa,bạn không thể mua hàng',
                    showConfirmButton: false,
                    timer: 1500 
                })
                return;
...
function infoPage(productId){
    ...
    window.location.href = `/pages/information.html?productId=${productId}`;
}